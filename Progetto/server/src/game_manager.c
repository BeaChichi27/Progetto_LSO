...
int game_request_rematch(int game_id, Client *client) {
    Game *game = game_find_by_id(game_id);
    if (!game || game->state != GAME_STATE_OVER) return 0;
    
    EnterCriticalSection(&game->mutex);
    
    if (game->rematch_state == REMATCH_NONE) {
        game->rematch_state = REMATCH_REQUESTED;
        game->rematch_requester = client;
        
        Client *opponent = (game->player1 == client) ? game->player2 : game->player1;
        if (opponent) {
            network_send_to_client(opponent, "REMATCH_REQUEST");
            network_send_to_client(client, "WAITING_REMATCH_RESPONSE");
        }
    }
